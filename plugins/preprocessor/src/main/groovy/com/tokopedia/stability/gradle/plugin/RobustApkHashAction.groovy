...
//                        createHashFile(assetsDir.getAbsolutePath(), Constants.STABILITY_APK_HASH_FILE_NAME, robustHash)
                        for (File file : packageTask.getInputs().getFiles().getAsFileTree()) {
                            if (file.getAbsolutePath().endsWith(".ap_")) {
                                try {
                                    createHashFile2(file.getAbsolutePath(), "assets/" + Constants.STABILITY_APK_HASH_FILE_NAME, robustHash);
//                                    System.out.println(">>>write hash:" + robustHash);
                        ...
                        createHashFile(assetsFileCollection.asPath, Constants.STABILITY_APK_HASH_FILE_NAME, robustHash)
                    }
                    //额外保存一份Constants.STABILITY_APK_HASH_FILE_NAME文件到build/output/robust，便于CI存储
                    String buildRobustDir = "${project.buildDir}" + File.separator + "$Constants.STABILITY_GENERATE_DIRECTORY" + File.separator
                    createHashFile(buildRobustDir, Constants.STABILITY_APK_HASH_FILE_NAME, robustHash)
                    return
                ...
                if (null != assets) {
                    ...
                    createHashFile(assets.absolutePath, Constants.STABILITY_APK_HASH_FILE_NAME, robustHash)
                } else {
                    // add robustHashFile to resourceFile
                    File robustHashFile = createHashFile(resourceFile.parentFile.absolutePath, Constants.STABILITY_APK_HASH_FILE_NAME, robustHash)
                    RobustApkHashZipUtils.addApkHashFile2ApFile(resourceFile, robustHashFile);
                }

                String buildRubustDir = "${project.buildDir}" + File.separator + "$Constants.STABILITY_GENERATE_DIRECTORY" + File.separator
                createHashFile(buildRubustDir, Constants.STABILITY_APK_HASH_FILE_NAME, robustHash)
