...
class ReadXML {
    ...
    public static void readXMl(String path) {
        robust = new XmlSlurper().parse(new File("${path}${File.separator}${Constants.STABILITY_XML}"))

        //Read the configured patch package name
        ...
        if (Config.supportProGuard&&(Config.mappingFilePath == null || "" == Config.mappingFilePath || !(new File(Config.mappingFilePath)).exists())) {
            throw new RuntimeException("Not found ${Config.mappingFilePath}, please put it on your project's stability dir or change your stability.xml !");
        }
