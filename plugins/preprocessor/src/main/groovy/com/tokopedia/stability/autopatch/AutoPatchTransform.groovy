...
import com.tokopedia.stability.utils.SmaliTool
...
class AutoPatchTransform extends Transform {
    ...
    AutoPatchTransform(Project target) {
        ...
        robust = new XmlSlurper().parse(new File("${project.rootDir.path}/${Constants.STABILITY_XML}"))
    ...
    def initConfig() {
        ...
        Config.robustGenerateDirectory = "${project.rootProject.buildDir.path}" + File.separator + "$Constants.STABILITY_GENERATE_DIRECTORY" + File.separator;
...
//        throw new RuntimeException("auto patch end successfully")
    ...
    def autoPatch(List<CtClass> box) {
        String patchPath = project.rootProject.buildDir.absolutePath + File.separator + Constants.STABILITY_GENERATE_DIRECTORY + File.separator
    ...
    def generatPatch(List<CtClass> box,String patchPath){
        if (!Config.isManual) {
            if (Config.patchMethodSignatureSet.size() < 1) {
                throw new RuntimeException(" patch method is empty ,please check your Modify annotation or use InlineModify.modify() to mark modified methods")
            }
