...
import com.tokopedia.stability.autopatch.ClassMapping;
import com.tokopedia.stability.autopatch.Config;
import com.tokopedia.stability.autopatch.NameManger;
import com.tokopedia.stability.autopatch.ReadMapping;
...
import static com.tokopedia.stability.autopatch.Config.invokeSuperMethodMap;
...
public class SmaliTool {
    ...
    private String dealWithSmaliLine(final String line, String fullClassName) {
        ...
        // Mainly for the new classes of @Add, you need to create obfuscation rules for them to prepare for obfuscation of method names in the future.
                ...
                if (packNameFromSmaliLine.size() > 0) {
                    ...
                    if (superClassMapping != null) {
                        for (String key : superClassMapping.getMemberMapping().keySet()) {
                            // Theoretically, .super appears earlier than .implement, and the inherited obfuscation rules should be more accurate, so whoever is advanced in map has priority here.
        ...
        for (int index = 0; packageNameList != null && index < packageNameList.size(); index++) {
            ...
            if (result.contains("invoke") &&
                ...
                // Adding "->" before and after is to avoid that when the class name contains the field name, the class name is mistakenly modified and the patch generation error is caused
                result = result.replaceFirst("->" + fieldName, "->" + getObscuredMemberName(packageNameList.get(index).replaceAll("/", "\\."), fieldName));
            }
        }

        // Handle the method name confusion of @Add new classes
        if (Config.newlyAddedClassNameList.contains(fullClassName)) {
            if (result.startsWith(".method ") &&
                ...
                // Note that getObscuredMethodSignure does not obfuscate the return value and cannot return directly after the following sentence. One last obfuscation
